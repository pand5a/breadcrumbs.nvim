...
local function get_three_level_path()
	local path = vim.fn.expand("%:p")
	local parts = vim.split(path, "/")

	if #parts >= 4 then
		return table.concat({ parts[#parts - 3], parts[#parts - 2], parts[#parts - 1], parts[#parts] }, "/")
	else
		return path
	end
end
...
M.get_filename = function()
	...
	if config.three_level_path then
		filename = get_three_level_path()
	end
